...
这是一个通过调用 API 或本地演示引擎实现会议纪要生成的 AI 提效工具，用于把会议转写文本整理成结构化纪要，并支持对纪要内容继续追问。项目包含前端交互页面、Node.js 后端逻辑、可选 LLM API 调用，以及无 API Key 时可本地运行的演示引擎。
...
- 体验方案：使用三栏工作台布局，把输入、洞察分析、追问与导出分区展示，适合现场演示。
