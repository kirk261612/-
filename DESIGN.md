...
- 示例载入：方便没有真实材料时快速体验。
- 会议模板：提供运营复盘、项目周会、需求评审和客户访谈四类输入模板。
- 介绍首页：作为默认第一页，集中展示产品价值、集成网格、能力矩阵和滚动演示。
- 功能工作台页：单独承载输入、生成、核心摘要和关键指标，保持实际操作路径清晰。
- 集成展示区：参考深色 SaaS 集成平台风格，用搜索入口和工具卡片表达纪要结果可进入知识库、待办、FAQ、看板和交付链路。
...
- 待办状态交互：点击状态可在待开始、进行中和已完成之间循环更新。
- 历史纪要页：最近 20 份纪要保存在本地浏览器，支持搜索、重新打开、删除和清空。
- 纪要追问页：根据已生成纪要继续问答。
- 笔记整理页：支持导入当前纪要或粘贴零散内容，整理为摘要、重点、行动项和幻灯片结构，并生成 `.pptx` 文件。
- 交付导出页：支持复制纪要、导出 Markdown 和 JSON。
- 日历导出：根据待办截止时间生成标准 `.ics` 文件，用于系统日历和提醒工具。
- 登录面板：桌面端在右上角展示登录入口，移动端把登录放入横向导航，演示账号状态存储在本地浏览器中。
- 订阅方案页：个人效率版、专业创作版和团队协作版均为付费方案，分别提供不同 AI 次数、PPT 额度和协作能力。
- 支付页：仅提供微信和支付宝收款码切换；用户确认完成支付后，在本地保存订阅状态。
- 访问控制：除介绍、登录、订阅和支付外，其它功能视图与操作都先检查订阅状态。
...
- 介绍首页：承担产品开场介绍、集成展示、能力矩阵和长滚动演示，是用户打开系统后的第一屏。
- 功能工作台：承担输入、生成和结果概览，是用户开始处理会议内容的主路径。
...
视觉风格参考 Oktar Integration 一类深色 SaaS 集成平台：黑色背景、细线分割、搜索栏、集成卡片网格、蓝绿高亮和玻璃质感面板，让页面更像面向企业工具连接的 AI 工作流产品。页面加入滚动进入视口的渐入、上移动效，以及导航栏滚动层次变化，增强向下浏览时的产品展示感。
